function event_say(e)
    local pop_tactics = tonumber(e.other:GetAccountBucket("pop.flags.rallos")) or 0
    
    if e.message:findi("hail") and pop_tactics == "1" then
        e.self:Say("It appears that you are ready to travel into the Elemental Planes. Beware many of the gods there have set up wards to protect them against assaults. Most I cannot speak about; however I do have information on the Circle of Protection that guards Xegony, in her [" .. eq.say_link("Plane of Air") .. "].")
    elseif e.message:findi("hail") then
        e.self:Say("Well met I am keeper of the Portal to the [" .. eq.say_link("Plane of Air") .. "].")
    end

    if e.message:findi("plane of air") and pop_tactics == "1" then
    ...
    elseif e.message:findi("elemental essences") and e.other:GetGlobal("mage_epic") == "6" then
