...
function do_the_spawn(e)
	...
	elseif phase_variable == 5 then
		...
		if quarm_variable == 0 and not eq.entity_list:GetMobByNpcTypeID(223201) then -- If left DZ and zone before P6 Complete lockout
...
function SpawnPhaseFour()
	...
	if expedition.valid then
		if saryrn_variable == 0 and not eq.entity_list:GetMobByNpcTypeID(223076) then
			eq.spawn2(223076, 0, 0, -320, -316, 358, 65) -- Saryrn
			UpdateFailTimer(60) -- 1 Hour per God
		end

		if tallon_variable == 0 and not eq.entity_list:GetMobByNpcTypeID(223077) then
			eq.spawn2(223077, 0, 0, 405, -84, 358, 384) -- Tallon Zek
			UpdateFailTimer(60) -- 1 Hour per God
		end

		if terris_variable == 0 and not eq.entity_list:GetMobByNpcTypeID(223075) then
			eq.spawn2(223075, 0, 0, -310, 307, 365, 190) -- Terris Thule
			UpdateFailTimer(60) -- 1 Hour per God
		end

		if vallon_variable == 0 and not eq.entity_list:GetMobByNpcTypeID(223078) then
...
function SpawnPhaseFive()
	...
	if expedition.valid then
		...
		if bertox_variable == 0 and not eq.entity_list:GetMobByNpcTypeID(223142) then
		...
		if cazic_variable == 0 and not eq.entity_list:GetMobByNpcTypeID(223166) then
		...
		if innoruuk_variable == 0 and not eq.entity_list:GetMobByNpcTypeID(223167) then
		...
		if rallos_variable == 0 and not eq.entity_list:GetMobByNpcTypeID(223168) then
