...
function do_the_spawn(e)
	...
	elseif phase_variable == 5 then
		...
		if quarm_variable == 0 and not eq.get_entity_list():GetMobByNpcTypeID(223201) then -- If left DZ and zone before P6 Complete lockout
		...
		if not eq.get_entity_list():GetMobByNpcTypeID(223228) then
...
function SpawnPhaseFour()
	...
	if expedition.valid then
		...
		if tallon_variable == 0 and not eq.get_entity_list():GetMobByNpcTypeID(223077) then
			eq.spawn2(223077, 0, 0, 405, -84, 358, 384) -- Tallon Zek
			UpdateFailTimer(60) -- 1 Hour per God
		end

		eq.debug("Terris check: variable=" .. terris_variable .. ", mob exists=" .. to_string(eq.is_npc_spawned({223075})))
		...
		if vallon_variable == 0 and not eq.get_entity_list():GetMobByNpcTypeID(223078) then
...
function SpawnPhaseFive()
	...
	if expedition.valid then
		...
		if bertox_variable == 0 and not eq.get_entity_list():GetMobByNpcTypeID(223142) then
		...
		if cazic_variable == 0 and not eq.get_entity_list():GetMobByNpcTypeID(223166) then
		...
		if innoruuk_variable == 0 and not eq.get_entity_list():GetMobByNpcTypeID(223167) then
		...
		if rallos_variable == 0 and not eq.get_entity_list():GetMobByNpcTypeID(223168) then
